<?php
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        $exist_user = $mysqli -> query("SELECT us.email FROM  USUARIOS us WHERE us.email = '".$email."'");
        if($exist_user -> num_rows > 0){
            ...
            if(!$stmt){
                die("ERROR AL PREPARAR LA QUEYR". $mysqli -> error);
            }
            $stmt -> bind_param("sssssi",$firstName,$lastName,$email,$password_hasheada,$profilePhotoUrl,$id_rol);
            if($stmt -> execute()){
...
<head>
    ...
    <title>Afegir Nou Client - BlueWave Hotels</title>
    <style>
        ...
        * {
        ...
        /* ---------------------------------- */
        /* 3. Estils del Formulari d'Afegir Client */
        /* ---------------------------------- */
        ...
        /* Camp d'adreça/text sencer (Mantinc la classe per si es necessita) */
        .form-group-full {
            grid-column: 1 / -1;
        }
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="form-section">
            ...
            <!-- FORMULARI - ACCIÓ: Utilitzem clients_add.php i mètode POST -->
            <form id="addClientForm" action="#" method="post">
                <div class="form-grid">
                    <!-- Fila 1: Nom, Cognoms -->
                    <div class="form-group">
                        <label for="firstName">Nom</label>
                        <input type="text" id="firstName" name="firstName" placeholder="P. ex.: Marc" required>
                    </div>

                    <div class="form-group">
                        <label for="lastName">Cognoms</label>
                        <input type="text" id="lastName" name="lastName" placeholder="P. ex.: Garcia Lòpez" required>
                    </div>

                    <!-- Fila 2: Correu Electrònic, Contrasenya (Nou camp) -->
                    <div class="form-group">
                        <label for="email">Correu Electrònic</label>
                        <input type="email" id="email" name="email" placeholder="client@exemple.com" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="url" id="profilePhotoUrl" name="profilePhotoUrl" placeholder="https://exemple.com/imatge.jpg">
                    </div>
                </div>

                <!-- Els camps DNI/Telèfon/Adreça s'han eliminat. -->

                <!-- Botons d'Acció -->
                <div class="form-actions">
                    <!-- RUTA: Torna al panell d'administració principal -->
                    <a href="./admin_panel.php" class="btn-cancel">Cancel·lar</a>
                    <button type="submit" class="btn-save">Guardar Client</button>
                </div>
            </form>
        </div>
    </div>
